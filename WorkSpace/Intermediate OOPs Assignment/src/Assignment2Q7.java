...
/**
 * 
 * abstract class DesertItem { abstract public int getCost(); public
 * List<String> getList() { List<String> list = new ArrayList<>();
 * list.add("Candy"); list.add("Cooky"); list.add("IceCream"); return list; }
 * static int totalAmount =0; } class Candy extends DesertItem { List<String>
 * itemList = new ArrayList<>(); public List<String> addItem(String name) {
 * itemList.add(name); return itemList; } public static int addCandies(int
 * candies){ totalAmount +=candies*60; return totalAmount; }
 * 
 * @Override public int getCost() {
 * 
 *           return totalAmount; }
 * 
 *           public List<String> getList() { return itemList; } } class Cookie
 *           extends DesertItem { List<String> itemList = new ArrayList<>();
 *           public List<String> addItem(String name) { itemList.add(name);
 *           return itemList; } public int addCookies(int cookies){ totalAmount
 *           += cookies*70; return totalAmount; }
 * 
 * @Override public int getCost() { totalAmount; }
 * 
 * @Override public List<String> getList() { return itemList; } } class IceCream
 *           extends DesertItem { List<String> itemList = new ArrayList<>();
 *           public List<String> addItem(String name) { itemList.add(name);
 *           return itemList; }
 * 
 *           public int addIceCreams(int iceCream){ totalAmount += iceCream*70;
 *           return totalAmount;}
 * 
 * @Override public int getCost() { 
 *           totalAmount; }
 * 
 * @Override public List<String> getList() { return itemList; } } class
 *           TotalCost extends DesertItem {
 * 
 * @Override public int getCost() { totalAmount; } } public class Assignment2Q7
 *           { static Candy objCandy = new Candy(); static Cookie objCookie =
 *           new Cookie(); static IceCream objIceCream = new IceCream(); static
 *           TotalCost objCost = new TotalCost(); public static void
 *           main(String[] args) { objCandy.addItem("Kimi");
 *           objCandy.addItem("Melody"); objCandy.addItem("Pulse");
 *           objCookie.addItem("Hide and Seek"); objCookie.addItem("GoodDay");
 *           objCookie.addItem("MariGold"); objIceCream.addItem("Amul");
 *           objIceCream.addItem("Kwality Wall's"); objIceCream.addItem("Arun");
 *           objCandy.addCandies(2); objCookie.addCookies(3);
